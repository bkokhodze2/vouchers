...
export default function Category({serverData}: any) {
  ...
  useEffect(() => {
    ...
    if (!!promo && Router?.query?.id) {
      axios
        .get(`${baseApi}/vouchers?contractId=662&categoryId=${Router?.query?.id}&isPromo=1&limit=100`)
        .then((res) => {
          setPromo(res.data)
        });
    }
    ...
    <>
      ...
      <div className={""}>
        ...
        <div className={"m-auto  mt-4 ph:mt-8 pb-[100px]"}>
          ...
          <div
            className={"mt-[40px] container m-auto mt-4 grid grid-flow-row-dense grid-cols-1 xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-x-4 gap-y-5 md:gap-y-[42px] xl:gap-x-[30px]"}>
            {
